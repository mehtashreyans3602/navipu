...
const AboutEventItem = ({
  ...
  return (
    <section className="  overflow-y-auto bg-[#beecdd90] backdrop-blur-sm border-[#00201990] border-2 active:border-2 w-[20.9rem] h-full flex flex-col p-4 gap-y-6 justify-between rounded-2xl cursor-pointer ">
      ...
      <section className="flex w-full items-center gap-2 justify-start">
        {image.map((image, index) => (
          <img
            ...
            className={`${index < 3 ? (index < 2 ? 'w-[30%]' : 'w-[20%]') : 'w-[10%]'}  overflow-x-hidden rounded-xl h-40 object-cover border-2 border-[#46492f] transition-all hover:scale-x-110 hover:duration-500  hover:w-[40%]`}
